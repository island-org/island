...
// If you use this binding you'll need to call 4 functions: ImGui_ImplXXXX_Init(), ImGui_ImplXXXX_NewFrame(), igRender() and ImGui_ImplXXXX_Shutdown().
...
//  2018-02-20: Inputs: Added support for mouse cursors (igGetMouseCursor() value and WM_SETCURSOR message handling).
...
struct ImVec2 makeImVec2(float x, float y)
{
    struct ImVec2 vec;
    vec.x = x;
    vec.y = y;
...
void ImGui_ImplGlfwGL3_RenderDrawData(struct ImDrawData* draw_data)
{
    ...
    glVertexAttribPointer(g_AttribLocationPosition, 2, GL_FLOAT, GL_FALSE, sizeof(struct ImDrawVert), (GLvoid*)IM_OFFSETOF(struct ImDrawVert, pos));
    glVertexAttribPointer(g_AttribLocationUV, 2, GL_FLOAT, GL_FALSE, sizeof(struct ImDrawVert), (GLvoid*)IM_OFFSETOF(struct ImDrawVert, uv));
    glVertexAttribPointer(g_AttribLocationColor, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(struct ImDrawVert), (GLvoid*)IM_OFFSETOF(struct ImDrawVert, col));
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, g_VboHandle);
        glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)ImDrawList_GetVertexBufferSize(cmd_list) * sizeof(struct ImDrawVert), (const GLvoid*)ImDrawList_GetVertexPtr(cmd_list, 0), GL_STREAM_DRAW);
...
int ImGui_ImplGlfwGL3_CreateFontsTexture()
{
    // Build texture atlas
    struct ImGuiIO* io = igGetIO();
...
int    ImGui_ImplGlfwGL3_Init(GLFWwindow* window, int install_callbacks, const char* glsl_version)
{
    g_Window = window;
    ...
    assert((int)strlen(glsl_version) + 2 < IM_ARRAYSIZE(g_GlslVersion));
...
void ImGui_ImplGlfwGL3_NewFrame()
{
    ...
    io->DisplaySize = makeImVec2((float)w, (float)h);
    io->DisplayFramebufferScale = makeImVec2(w > 0 ? ((float)display_w / w) : 0, h > 0 ? ((float)display_h / h) : 0);
    ...
    if (glfwGetWindowAttrib(g_Window, GLFW_FOCUSED))
    {
        ...
        else
        {
            ...
            io->MousePos = makeImVec2( (float)mouse_x, (float)mouse_y);
        }
    }
    else
    {
        io->MousePos = makeImVec2( -FLT_MAX,-FLT_MAX );
    }
