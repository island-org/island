...
static int anyActive()
{
    return g_state.active != 0;
}

static int isActive(unsigned int id)
{
    return g_state.active == id;
}

static int isHot(unsigned int id)
{
    return g_state.hot == id;
}

static int inRect(int x, int y, int w, int h, int checkScroll)
{
...
{
    g_state.leftPressed = 0;
    g_state.leftReleased = 0;
    g_state.scroll = 0;
}
...
{
    int left = (mbut & IMGUI_MBUT_LEFT) != 0;
...
static int g_insideScrollArea = 0;
...
int imguiBeginScrollArea(const char* name, int x, int y, int w, int h, int* scroll)
{
    ...
    g_insideScrollArea = inRect(x, y, w, h, 0);
...
int imguiSlider(const char* text, float* val, float vmin, float vmax, float vinc, int enabled)
{
    ...
    over = enabled && inRect(x+m, y, SLIDER_MARKER_WIDTH, SLIDER_HEIGHT, 1);
    res = buttonLogic(id, over);
    valChanged = 0;
...
void imguiSeparatorLine()
{
    int x, y, w, h;
    x = g_state.widgetX;
    y = g_state.widgetY - DEFAULT_SPACING*2;
    w = g_state.widgetW;
    h = 1;
